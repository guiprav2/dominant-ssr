...
function createDominant(win) {
  if (!win) return {};
  var window = win.window;
  var document = window.document || globalThis.document;
  var Node = window.Node || globalThis.Node;
  var navigator = window.navigator || globalThis.navigator || { userAgent: '' };
  var MutationObserver = window.MutationObserver || globalThis.MutationObserver;
  var requestAnimationFrame =
    (window.requestAnimationFrame ||
      globalThis.requestAnimationFrame ||
      function (cb) {
        return (window.setTimeout || globalThis.setTimeout)(cb, 16);
      }).bind(window);
  ...
  async function updateSync(di) {
    if (globalThis !== window) await Promise.resolve();
    di = di || {};
    di.boundNodes = di.boundNodes || boundNodes;
...
const dominant = createDominant();

dominant.ssr = function ssr(windowInstance) {
  if (!windowInstance || !windowInstance.document) {
    throw new TypeError('dominant.ssr(window) requires a valid window instance');
  }
  return createDominant(windowInstance);
};
